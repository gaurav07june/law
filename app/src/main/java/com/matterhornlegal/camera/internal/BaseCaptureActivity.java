...
import com.matterhornlegal.camera.TimeLimitReachedException;
...
public abstract class BaseCaptureActivity extends AppCompatActivity implements BaseCaptureInterface {
    ...
    @Override
    public final void useMedia(final String uri) {
        if (uri != null) {
            if(startAsResult())
            {
    ...
    public boolean startAsResult() {
        return getIntent().getBooleanExtra(CameraIntentKey.START_AS_RESULT, false);
    }
    ...
    public int iconStillshot() {
        return getIntent().getIntExtra(CameraIntentKey.ICON_STILL_SHOT, R.drawable.camera_btn);
    }
    ...
    @Override
    public boolean defaulttoFrontFacing() {
        return getIntent().getBooleanExtra(CameraIntentKey.DEFAULT_TO_FRONT_FACING, false);
    }
   /* @Override
